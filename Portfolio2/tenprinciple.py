...
# Apply PCA to reduce the data to the top 10 principal components
...
final_output_df.to_csv('svm_full_output_with_pca.csv', index=False)

# Additionally, save the explained variance ratio of the principal components
pca_explained_variance = pd.DataFrame({
    'Principal Component': [f'PC{i+1}' for i in range(10)],
    'Explained Variance Ratio': pca.explained_variance_ratio_
})
pca_explained_variance.to_csv('pca_explained_variance.csv', index=False)

print("All outputs and PCA explained variance have been saved to svm_full_output_with_pca.csv and pca_explained_variance.csv")
